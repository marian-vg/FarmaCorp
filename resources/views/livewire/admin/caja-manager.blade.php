<div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl max-w-none">
                ...
                <flux:table>
                    ...
                    <flux:table.rows>
                        @forelse($this->cajas as $caja)
                            <flux:table.row :key="'gestion-'.$caja->id">
                                <flux:table.cell>
                        ...
                        @empty
                            <flux:table.row>
                                <flux:table.cell colspan="5" class="text-center py-10 text-zinc-500">No hay cajas abiertas.</flux:table.cell>
                            </flux:table.row>
                        @endforelse
                    </flux:table.rows>
                </flux:table>
            ...
            <div class="w-full overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-700">
                <flux:table>
                    <flux:table.columns>
                        ...
                        <flux:table.column align="end">Monto Final</flux:table.column>
                        <flux:table.column align="end">Acciones</flux:table.column>
                    </flux:table.columns>
                    <flux:table.rows>
                        @forelse($this->historialCajas as $caja)
                            <flux:table.row :key="'historial-'.$caja->id">
                                <flux:table.cell>{{ $caja->user->name }}</flux:table.cell>
                                <flux:table.cell>{{ $caja->fecha_apertura->format('d/m H:i') }}</flux:table.cell>
                                <flux:table.cell>{{ $caja->fecha_cierre->format('d/m H:i') }}</flux:table.cell>
                                <flux:table.cell align="end" class="font-bold text-indigo-600">${{ number_format($caja->monto_final, 2) }}</flux:table.cell>
                                <flux:table.cell align="end">
                        ...
                        @empty
                            <flux:table.row>
                                <flux:table.cell colspan="5" class="text-center py-10">No hay registros históricos.</flux:table.cell>
                            </flux:table.row>
                        @endforelse
                    </flux:table.rows>
                </flux:table>
    ...
    <flux:modal name="detalle-caja-panel" variant="side" class="min-w-[45rem]">
        @if($cajaSeleccionada)
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <flux:card class="bg-indigo-50 dark:bg-indigo-900/20">
                        ...
                        <flux:heading size="md" class="text-indigo-600">${{ number_format($this->saldoActual, 2) }}</flux:heading>
                    </flux:card>
                    <flux:card>
                ...
                {{-- Desglose MP --}}
                <div class="mt-4">
                    <flux:heading size="sm" class="mb-2">Desglose por Medios de Pago</flux:heading>
                    <div class="grid grid-cols-2 gap-2">
                        @foreach($this->totalesPorMedio as $nombre => $total)
                            <div class="p-2 border rounded text-sm flex justify-between">
                                <span>{{ $nombre }}</span>
                ...
                <flux:table>
                    ...
                    <flux:table.rows>
                        @foreach($cajaSeleccionada->movimientos()->with('user')->get() as $mov)
                            <flux:table.row>
                                <flux:table.cell>{{ $mov->fecha_movimiento->format('H:i') }}</flux:table.cell>
                                <flux:table.cell>{{ $mov->user->name }}</flux:table.cell>
                                <flux:table.cell align="end" class="{{ $mov->tipo_movimiento === 'INGRESO' ? 'text-green-600' : 'text-red-600' }}">
    ...
    {{-- MODAL CIERRE FINAL --}}
    <flux:modal name="confirm-admin-close-caja" class="min-w-[32rem]">
